...
class ParserService:
    ...
    async def _get_browser_context(self, p, user_id, headless=True):
        ...
        if not user_data_dir:
            ...
            await self.db.set_browser_session(user_id, user_data_dir)
    ...
    def _find_attachments_recursively(self, obj, seen_links, results):
        """
        Рекурсивно ище вложения и ссылки. 
        Теперь ЛУЧШЕ извлекает оригинальные файлы учителей из Мосрега.
        """
        if not obj: return
        ...
        if isinstance(obj, list):
            ...
            name = obj.get('file_name') or obj.get('name') or obj.get('title') or \
                   obj.get('filename') or obj.get('text') or obj.get('original_name') or 'Файл'
            
            # 2. Извлечение ссылки
            link = obj.get('link') or obj.get('url') or obj.get('download_url') or \
                   obj.get('file_url') or obj.get('download_link') or \
                   obj.get('web_link') or obj.get('path') or obj.get('downloadUrl')
            ...
            if link and isinstance(link, str) and (link.startswith('http') or link.startswith('/')):
                ...
                # Игнорируем блокируемые домены
                if not any(domain in link.lower() for domain in BLOCKLIST):
                    if link not in seen_links:
                        # Разрешаем ссылки с портала Mosreg даже без расширения (это файлы учителей)
                        is_portal_file = "mosreg.ru" in link.lower() and ("/files/" in link or "/attachments/" in link or "/v2/files/" in link)
                        is_file = any(link.lower().endswith(ext) for ext in ['.pdf', '.doc', '.docx', '.xls', '.xlsx', '.ppt', '.pptx', '.jpg', '.png', '.zip', '.jpeg', '.txt'])
                        
                        # Если это НЕ файл портала и НЕ стандартный файл, и при этом в конце длинный ID (мусор) - отсекаем
                        if not is_file and not is_portal_file and re.search(r'/\d{7,}/?$', link): 
                             link = None
                        ...
                        if link:
                            ...
                            if icon not in display_name: display_name = f"{icon} {display_name}"
            ...
            # 3. Рекурсия по системным полям, КРОМЕ общего уровня урока (мы вызываем это только для homework)
    ...
    async def get_mosreg_schedule(self, access_token, student_id, date_str, mesh_id=None, force_refresh=False):
        ...
        if not force_refresh:
            ...
            # Принудительное обновление - удаляем старый кеш
    ...
    def _parse_structural_diary(self, raw_payload):
        ...
        for ev in raw_payload:
            ...
            source = ev.get('source', 'PLAN')
            # Базовые данные
            subject = ev.get('subject_name') or ev.get('title') or ev.get('subject', {}).get('name', 'Урок')
            
            # Если это праздник/каникулы (только если нет названия предмета или спец. источник)
            is_holiday = source in ['EVENTS', 'AE', 'ORGANIZER'] or any(kw in subject.lower() for kw in ['каникулы', 'праздник', 'выходной'])
            ...
            # Извлечение домашнего задания и ЦДЗ
            hw_text_list = []
            attachments = []
            seen_links = set()
            ...
            if isinstance(hw_data, dict):
                ...
                # Ищем файлы ТОЛЬКО в объекте домашнего задания (как просил юзер)
                self._find_attachments_recursively(hw_data, seen_links, attachments)
            elif isinstance(hw_data, list):
                for h_item in hw_data:
                     descs = h_item.get('descriptions') or h_item.get('description') or []
                     if isinstance(descs, list): hw_text_list.extend([str(d) for d in descs if d])
                     self._find_attachments_recursively(h_item, seen_links, attachments)
            ...
            hw_hash = hashlib.md5(hw_raw_data.encode()).hexdigest()[:16]
    ...
    async def _solve_videouroki(self, user, test_url, status_callback, screenshot_callback):
        async with async_playwright() as p:
            ...
            try:
                ...
                await self.db.add_test_score(user['user_id'], test_url, result_text)
                await self.db.add_test_history(user['user_id'], test_url, result_text)
            ...
            try:
                ...
                while True:
                    ...
                    if final_score:
                        ...
                        await self.db.add_test_score(user['user_id'], test_url, res_text)
                        await self.db.add_test_history(user['user_id'], test_url, res_text + " (МЭШ)")
